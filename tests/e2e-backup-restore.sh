#!/bin/bash
# End-to-end tests for the authelia-traefik-letsencrypt-docker-compose backup + restore flow.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-authelia}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-authelia-traefik-letsencrypt-docker-compose.yml}"
...
: "${AUTHELIA_POSTGRES_BACKUPS_PATH:=/srv/authelia-postgres/backups}"
: "${AUTHELIA_POSTGRES_BACKUP_NAME:=authelia-postgres-backup}"
: "${AUTHELIA_BACKUP_INTERVAL:=24h}"
: "${AUTHELIA_DB_NAME:=autheliadb}"
: "${AUTHELIA_DB_USER:=autheliadbuser}"

BACKUPS_PATH="${AUTHELIA_POSTGRES_BACKUPS_PATH%/}"
BACKUP_PREFIX="${AUTHELIA_POSTGRES_BACKUP_NAME}"
BACKUP_EXT=".gz"
INTERVAL="${AUTHELIA_BACKUP_INTERVAL}"
...
# The database password lives in a secret file, not in the container
# environment: every client call exports it first.
db_query() {
  backups_sh "export PGPASSWORD=\"\$(cat /run/secrets/storage_password)\"; psql -h $DB_HOST -p 5432 -U $DB_USER -d $DB_NAME -tAc \"$1\""
}
db_ready() {
...
test_restore_roundtrip() {
  ...
  echo "  restoring the baseline with ./authelia-restore-database.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./authelia-restore-database.sh "$(basename "$baseline")" \
