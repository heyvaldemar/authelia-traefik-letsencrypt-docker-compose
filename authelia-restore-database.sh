#!/usr/bin/env bash
# authelia-restore-database.sh [backup-file-name]
#
# Replaces the Authelia database with one of the dumps the backups service wrote.
#
#   ./authelia-restore-database.sh               list and ask
#   ./authelia-restore-database.sh <file-name>   restore that one
#
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than authelia.
set -Eeuo pipefail
...
DIR="$(env_of AUTHELIA_POSTGRES_BACKUPS_PATH)"; NAME="$(env_of AUTHELIA_POSTGRES_BACKUP_NAME)"
DB_NAME="$(env_of AUTHELIA_DB_NAME)"; DB_USER="$(env_of AUTHELIA_DB_USER)"
...
if ! docker exec "$BKP" sh -c "(set -o pipefail) 2>/dev/null && set -o pipefail; set -eu
    export PGPASSWORD=\"\$(cat /run/secrets/storage_password)\"
  ...
  echo "error: the restore failed part-way. The database may now be empty: restore another backup before using Authelia." >&2
  exit 1
fi
